...
//父子进程若只进行读操作则共用一份空间 若进行写操作谁写谁单独复制空间 父子进程之间互不影响
//fork后父子进程区别：fork返回值不同(父进程返回子进程pid 子进程返回0) pid不同 ppid不同 未决信号和文件锁不继承 资源利用率清零
//调度器调度策略决定哪个进程先运行

//init进程：即1号进程 是所有进程的祖先进程
//孤儿进程：父进程结束 子进程未结束 子进程成为孤儿进程 会被init进程接管
//僵尸进程：子进程结束 父进程未回收子进程pid 子进程成为僵尸进程
