#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
#include <errno.h>
#include <poll.h>

#define F1 "/dev/tty11"
#define F2 "/dev/tty12"

//poll以文件描述符为单位组织事件 数组形式存储fd

//优点
//可移植
//fd数量不受限
//速度快

//缺点
//fd用户态与内核态复制开销大
//函数返回后需要遍历所有fd 效率低

enum
{
    STATE_R,
    STATE_W,
    STATE_AUTO,
    STATE_EX,
    STATE_T
};

struct fsm_st
{
    int state;
    int src_fd;
    int dst_fd;
    char buf[BUFSIZ];
    size_t read_size;
    size_t pos;
    char* err;
};

void FsmDriver(struct fsm_st* fsm)
{
    size_t write_size;

    switch(fsm->state)
    {
        case STATE_R:
            fsm->read_size = read(fsm->src_fd, fsm->buf, BUFSIZ);

            if(fsm->read_size == 0)
            {
                fsm->state = STATE_T;
            }
            else if(fsm->read_size < 0)
            {
                if(errno == EAGAIN)
                {
                    fsm->state = STATE_R;
                }
                else
                {
                    fsm->err = "read";
                    fsm->state = STATE_EX;
                }
            }
            else
            {
                fsm->pos = 0;
                fsm->state = STATE_W;
            }
            break;

        case STATE_W:
            write_size = write(fsm->dst_fd, fsm->buf + fsm->pos, fsm->read_size);

            if(write_size < 0)
            {
                if(errno == EAGAIN)
                {
                    fsm->state = STATE_W;
                }
                else
                {
                    fsm->err = "write";
                    fsm->state = STATE_EX;
                }
            }
            else
            {
                fsm->pos += write_size;
                fsm->read_size -= write_size;

                if(fsm->read_size == 0)
                {
                    fsm->state = STATE_R;
                }
                else
                {
                    fsm->state = STATE_W;
                }
            }
            break;

        case STATE_EX:
            perror(fsm->err);
            fsm->state = STATE_T;
            break;

        case STATE_T:
            exit(-1);
            break;
    }
}

void Relay(int fd1, int fd2)
{
    int fd1_save, fd2_save;
    struct fsm_st fsm12, fsm21;
    struct pollfd poll_fd[2];   //每一个文件描述符对应两个事件位图：关注的事件、发生的事件

    //保存旧文件描述符状态 增加非阻塞标志 若没读到数据则设置errno为EAGAIN
    fd1_save = fcntl(fd1, F_GETFL);
    fcntl(fd1, F_SETFL, fd1_save | O_NONBLOCK);
    fd2_save = fcntl(fd2, F_GETFL);
    fcntl(fd2, F_SETFL, fd2_save | O_NONBLOCK);

    //初始化状态机
    fsm12.state = STATE_R;
    fsm12.src_fd = fd1;
    fsm12.dst_fd = fd2;
    fsm21.state = STATE_R;
    fsm21.src_fd = fd2;
    fsm21.dst_fd = fd1;

    //初始化pollfd
    poll_fd[0].fd = fd1;
    poll_fd[1].fd = fd2;

    //执行
    while(fsm12.state != STATE_T || fsm21.state != STATE_T)
    {
        //布置监视任务
        poll_fd[0].events = 0;
        if(fsm12.state == STATE_R)
        {
            poll_fd[0].events |= POLLIN;
        }
        if(fsm21.state == STATE_W)
        {
            poll_fd[0].events |= POLLOUT;
        }
        poll_fd[1].events = 0;
        if(fsm21.state == STATE_R)
        {
            poll_fd[1].events |= POLLIN;
        }
        if(fsm12.state == STATE_W)
        {
            poll_fd[1].events |= POLLOUT;
        }

        //监视
        if(fsm12.state < STATE_AUTO || fsm21.state < STATE_AUTO)
        {
            //结构体地址 结构体大小 超时时间 返回发生事件个数
            while(poll(poll_fd, 2, -1) < 0)     //超时(阻塞)监视
            {
                if(errno == EINTR)  //被信号打断 假错
                {
                    continue;
                }
                else
                {
                    perror("poll");
                    exit(-1);
                }
            }
        }

        //查看监视结果
        if((poll_fd[0].revents & POLLIN) ||
           (poll_fd[1].revents & POLLOUT) ||
           fsm12.state > STATE_AUTO)
        {
            FsmDriver(&fsm12);
        }
        if((poll_fd[1].revents & POLLIN) ||
           (poll_fd[0].revents & POLLOUT) ||
           fsm12.state > STATE_AUTO)
        {
            FsmDriver(&fsm21);
        }
    }

    //还原旧文件描述符状态
    fcntl(fd1, F_SETFL, fd1_save);
    fcntl(fd2, F_SETFL, fd2_save);
}

int main()
{
    int fd1, fd2;

    fd1 = open(F1, O_RDWR);
    if(fd1 < 0)
    {
        perror("open");
        exit(-1);
    }
    write(fd1, "fd1\n", 4);

    fd2 = open(F2, O_RDWR);
    if(fd2 < 0)
    {
        perror("open");
        exit(-1);
    }
    write(fd2, "fd2\n", 4);

    Relay(fd1, fd2);

    close(fd2);
    close(fd1);

    return 0;
}
